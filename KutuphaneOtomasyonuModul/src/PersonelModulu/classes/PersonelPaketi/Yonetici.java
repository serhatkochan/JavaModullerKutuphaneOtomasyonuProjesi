...
public class Yonetici implements IYonetici{ // imp yonetici
    ...
    @Override
    public void programiKapa() { // kalıtım aldığımız metot
        System.exit(0);
    }
    ...
    public static int maxCalisanSayisiDondur(){
        CalisanSayisiKontrol calisanSayisiKontrol = new CalisanSayisiKontrol();
        return calisanSayisiKontrol.getMax_calisan_sayisi();
    }
    public static int calisanSayisiDondur(){
                ...
                try {
                    Statement statement = connection.createStatement();
                    ResultSet resultSet = statement.executeQuery("select count(*) from tbl_personel where gorevi='Çalışan'");
